...
class TrainDetails:
    def __init__(self,root,From=None,To=None,Train_Number=None,Date=None,Day=None):
        ...
        box_frame = Frame(root, bg="white", height=600, width=1200)
    ...
    # Retrieve the IRCTC ID entered by the user
        ...
        try:
            ...
            sql_query = "SELECT Train_Name FROM Train WHERE Train_Number = %s"
    ...
    # Fetch classes and prices from the database
        ...
        from_city = float(self.get_distance(From))
        to_city = float(self.get_distance(To))
        km = abs(from_city - to_city)
        if train_data:
    ...
    def get_distance(self, city):
        try:
            ...
            # Define the SELECT query to fetch classes and prices from the Train table
            sql_query = "SELECT Distance_to_Next_Station FROM Stations WHERE Station_Name = %s"
