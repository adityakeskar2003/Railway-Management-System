...
class ADD_DRIVER:
    ...
    def __init__(self,root,initial_values=None):
        ...
        box_frame = Frame(root, bg="white", height=600, width=730)
        ...
        try:
            ...
            if table_exists:
                ...
                # If driver exists, update the entry
                    sql_query = "UPDATE Driver SET FirstName=%s, LastName=%s, DOB=%s, Address=%s, City=%s, State=%s, Year_of_Join=%s WHERE IRCTC_ID=%s"
                    values = (first_name, last_name, dob, address, city, state, year_of_join, irctc_id)
                else:
                    # If driver does not exist, insert a new entry
                    sql_query = "INSERT INTO Driver (FirstName, LastName, IRCTC_ID, DOB, Address, City, State, Year_of_Join) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
