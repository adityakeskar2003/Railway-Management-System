...
class ADD_STATIONS:
    ...
    def __init__(self,root,initial_values=None):
        ...
        box_frame = Frame(root, bg="white", height=600, width=500)
    ...
    # Retrieve values from entry widgets
        ...
        try:
            ...
            # Check if the table exists
            cursor.execute("SHOW TABLES LIKE 'Stations'")
            table_exists = cursor.fetchone()

            if table_exists:
                # Check if the station already exists
                cursor.execute("SELECT * FROM Stations WHERE Station_Name = %s", (Station.upper(),))
                existing_station = cursor.fetchone()
                
                if existing_station:
                    # If station exists, update the entry
                    sql_query = "UPDATE Stations SET Distance_to_Next_Station = %s, State = %s WHERE Station_Name = %s"
                    values = (distance, state.upper(), Station.upper())
                else:
                    # If station does not exist, insert a new entry
                    sql_query = "INSERT INTO Stations (Station_Name, Distance_to_Next_Station, State) VALUES (%s, %s, %s)"
