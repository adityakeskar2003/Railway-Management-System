...
class ADD_TRAIN:
    def __init__(self,root,initial_values=None):
        ...
        box_frame = Frame(root, bg="white", height=630, width=915)
        box_frame.place(x=80, y=160)

        pnr_heading = Label(box_frame, text="ADD TRAIN DETAILS", font=('Helvetica', 20, 'bold'), bg="white", fg="navy blue")
        pnr_heading.place(x=280, y=40)
    ...
    def get_driver_names(self):
        try:
            ...
            cursor.execute("SELECT IRCTC_ID, FirstName, LastName FROM Driver")
        ...
        try:
            ...
            if table_exists:
                ...
                # If driver exists, update the entry
                    sql_query = "UPDATE Train SET Train_Name=%s, Source_Station=%s, Source_State=%s, Destination_Station=%s, Destination_State=%s, StartTime=%s, EndTime=%s, Days=%s WHERE Train_Number=%s"
                    ...
                    sql_query = "INSERT INTO Train (Train_Number,Train_Name, Source_Station, Source_State, Destination_Station, Destination_State, StartTime, EndTime,Days) VALUES (%s, %s, %s, %s, %s, %s, %s,%s,%s)"
